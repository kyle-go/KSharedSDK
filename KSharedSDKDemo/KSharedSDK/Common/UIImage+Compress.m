...
{
    if (self.size.width > sizewidth) {
        float scale = self.size.width / sizewidth;
        UIImage *imagetemp = [UIImage imageByScalingAndCroppingForSize:CGSizeMake(self.size.width/scale, self.size.height/scale) sourceImage:self];
        return imagetemp;
    }
    return self;
}
